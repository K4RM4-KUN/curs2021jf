        ...
        <table border = 1 style="border-color:#0F9582; background-color:#AAD0E8;">
            ...
            if ($result->num_rows >= 0){
                while($product = $result->fetch_assoc()){
                    $tmpId = $product["id"];
                    echo '<tr>
                            <form enctype="multipart/form-data" method="post" id="changeUserData" name="'.$product["id"].'">
                                <td><input name="id" style="text-align: center;" size="1" value="'.$product["id"].'" readonly></td>
                    ...
                    echo'<td><form><input type="hidden" name="proid" style="text-align: center;" size="1" value="'.$product["id"].'"></br><input name="cat_nom" type="text" maxlength="20" placeholder="Añadir categoria" size="15"><input type="submit" name="add_cat" value="Añadir"><table>';
                    while($product2 = $result2->fetch_assoc()){
                        echo '<tr><td><input type="hidden" name="catid" style="text-align: center;" size="1" value="'.$product2["id"].'"><a>-'.$product2["nombre"].'</a></td><td><input type="submit" name="delete_cat" value="Eliminar"></td></tr>';
                    }echo'</form></table></td>';
                            echo'<td><input type="file" name="pr_imagen"></td>
                                <td><input type="submit" value="Modificar" name="modify"></td>
